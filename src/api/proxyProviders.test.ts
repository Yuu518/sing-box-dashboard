...
it("does not subscribe on unsupported servers", async () => {
  const daemon = api();
  vi.spyOn(daemon, "serverInfo").mockResolvedValue({ version: "old", apiVersion: 4, proxyProvidersSupported: false });
...
it("publishes provider snapshots and cancels the stream when the view closes", async () => {
  const daemon = api();
  vi.spyOn(daemon, "serverInfo").mockResolvedValue({ version: "new", apiVersion: 5, proxyProvidersSupported: true });
